...
function inPageExtract(searchQuery) {
  ...
  function extractFromCard(cardNode, platformId) {
    ...
    let title; 
    // 2. Title extraction (Priority: specific slot/testid -> img alt -> specific title classes -> h2-h5)
    ...
    return {
      ...
      price,
      mrp: mrp || price,
      brand: platformId === "instamart" ? "Swiggy Instamart" : (platformId === "zepto" ? "Zepto" : (platformId === "blinkit" ? "Blinkit" : "Amazon Now (Tez)")),
      quantity,
      image,
          ...
          if (cleanT && price > 0 && !isBadTitle(cleanT)) {
            if (!candidates.some(c => c.title === cleanT && c.price === price)) {
              candidates.push({
                ...
                brand: platformId === "instamart" ? "Swiggy Instamart" : (platformId === "zepto" ? "Zepto" : (platformId === "blinkit" ? "Blinkit" : "Amazon")),
  ...
  // 2. PDP Handling
  if (isPDP) {
    const pdpTitle = document.querySelector('h1[data-testid*="name"], h1[data-testid*="title"], h1#title span');
